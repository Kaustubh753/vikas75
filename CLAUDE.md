...
- `src/lib/ai-judge.ts` — Dual-mode judge; the live path is a **debiased, multi-call** judge.
  - Live model id is `JUDGE_MODEL = 'claude-haiku-4-5'` (used when `ANTHROPIC_API_KEY` present) — a deliberate owner decision trading some judging nuance for ~3× lower cost. Never append a date suffix to the id; switching tiers (e.g. back to a Sonnet model) is this one constant, but read the cache note in the Cost bullet first.
  ...
  - Cost: three calls per round instead of one, at Haiku 4.5 rates — about 4¢ for a 3-round game with 5 players, ~3¢ per round at a full 20-player table. The ~1.5k-token system prompt still carries `cache_control`, but **Haiku 4.5's minimum cacheable prefix is 4096 tokens, so it does not cache on this model** — `cacheRead=0` in the per-call log is expected; the marker is kept so a future Sonnet/Opus-tier switch (minimum 1024) re-engages caching with no code change. The problem↔scheme mapping is compiled once at module load into an in-memory cache (`ON_BRIEF_BY_CHALLENGE`) and never enters the prompt — each answer carries only an ~8-token yes/no flag, vs ~350 tokens to paste the mapping.

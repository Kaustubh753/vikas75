...
### Explore Components
- `src/components/explore/ExplorePage.tsx` — The deck grid, search and tabs. Tapping a card opens its **scheme guide** (`SchemeDetailSheet`) directly: the old text panel restated exactly what the card face already prints (name, Hindi name, description, key points), so it was a second stop before the only screen that adds anything. `CardModal` survives as the fallback for a card with no guide (`hasSchemeDetail` false — currently only `s018`).
- `src/components/explore/SchemeDetailSheet.tsx` — Full-screen scheme guide. Three things to preserve:
  - **It fits.** The pages are 921×1650, far taller than a phone, so full-width rendering dropped the reader into a fragment of a page they hadn't seen. The default view contains the whole page in the viewport; tapping (or the Zoom button) switches to the page's own 921px width, where scrolling and panning belong. Capping the zoom at viewport width is not a zoom — the text stays unreadable, which is the only reason to zoom.
  - **It loads.** `unoptimized` + `priority`: a static CDN fetch (~230 ms in dev) instead of an optimiser round-trip.
  - Escape is handled in **capture** phase so one keypress closes the sheet, not the card modal behind it; and the loading placeholder is a background on the wrapper, never `display: none` on the image — a hidden `<img>` is never laid out, so it would never load and never reveal itself.
...
- `src/app/globals.css` — App-wide visual concerns: the Tailwind import and `@theme`, base/body styling and safe-area padding, focus-visible rings, the iOS input-zoom guard, the `prefers-reduced-motion` collapse (durations go to ~0 rather than `animation: none`, so forwards-filled animations still land on their end state instead of leaving content invisible), and the shared keyframes/utilities (`fade-in`, `slide-up`, `bounce-in`, `slam-in`, `confetti-fall`, `skeleton-pulse`). Keep it to animations something actually uses — a batch of never-referenced ones accumulated here once already.

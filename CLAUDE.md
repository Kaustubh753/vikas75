...
npm run test:filter     # unit tests for the profanity filter / name sanitiser (src/lib/word-filter.ts)

node scripts/build-scheme-details.mjs <pages-dir>            # asset-only: rebuild the English guides
node scripts/build-scheme-details.mjs <pages-dir> --lang hi  # …and the Hindi ones
```

There is **no test framework**. `npm test` runs three plain-node scripts (no runner, no API key; Node ≥ 22.18 for native type stripping, declared in `engines`), each covering a pure module that a bug would otherwise reach production through:
...
- `scripts/test-word-filter.mjs` — `src/lib/word-filter.ts`. Half of it is a **false-positive** list: this deck is about Indian government schemes, so `Gandhi` (gand + hi), `Assam`, `assets`, `class` and `Chandigarh` must survive untouched — censoring one of those on a projector is worse than missing a swear word. The other half asserts the inflections (`fucking`, `assholes`, `chutiye`, `bhosdike`) that used to pass uncensored, plus the ordering property that `filterText` **expands** its input, so callers must truncate after it.
...
`scripts/build-scheme-details.mjs` is a hand-run asset step, not part of the build or CI: it re-encodes the Explore scheme guides from a folder of exported PDF pages (`page02.jpg`…, matching the PDF's own page numbers) and exits non-zero if `src/lib/scheme-details.ts`'s id list has drifted from the map it just built against. `--lang hi` switches it to the Hindi deck (`context/scheme_details_map_hi.json` → `public/scheme-details-hi/`); both decks resample to exactly 921×1650 so the reader's zoom arithmetic has one asset width to know about. It imports `sharp`, which is not a declared dependency — it is present only transitively via Next.
...
- Host-only actions (`advance`, `update-settings`, `end-game`, `kick-player`, `music-toggle`) are gated by matching the raw `hostId` from the body against `room.hostId` (403 otherwise). All of them prove it **before** queuing for the room lock — `hostPreCheck` does the unlocked read, and also charges the per-host 120/min bucket, but only *after* the credential matches so a stranger cannot burn the real host's quota. The check inside the lock stays: the pre-read snapshot can be stale by the time the lock is granted, so it is a filter, never the authority.
...
- `src/lib/redis.ts` — Transparent Redis / in-memory fallback.
  ...
  - Exports: `getRoom`, `setRoom`, `deleteRoom`, `listActiveRooms`, `createRoomIfAbsent` (SET NX — claiming a room code and writing it must be ONE operation, or two hosts drawing the same candidate both see it free and the second write destroys the first's room), plus `acquireLock` (returns an ownership token, or null) / `releaseLock(key, token)` (used by `withRoomLock`).
...
- `src/lib/word-filter.ts` — `filterText` (profanity) and `sanitizeName` (display names). Three properties matter. It **expands** what it normalises (NFKC turns one `㌖` into six characters), so **every caller must truncate AFTER calling it, never before** — slicing first let a 30-character name store 180 and a 200-character explanation store 1200 while still counting as one word. Roots carry **explicit** suffix sets, never a wildcard, because `gand` + a `.{0,2}` tail censors *Gandhi*. And `sanitizeName` strips control/format/bidi characters and rejects names with nothing readable in them (Hangul Filler NFKC-folds to U+1160, which Unicode calls a letter, so it passes a naive "has a letter?" test). The judge's own output goes through `filterText` too — the prompt asks it to quote players, which is a sanctioned path from untrusted text to the projector.
...
- `src/lib/use-countdown.ts` — `useCountdown(endsAt, fallback)`: seconds left, derived from the clock at render time and re-rendered on a **500 ms** timer. Half-second ticks matter even though the value is whole seconds: sampling `Math.ceil(remaining/1000)` once a second aliases against the second boundary, so the number sticks and then skips. Four components had their own copy (`ProjectorSubmission`, `ProjectorChallengeReveal`, `MobileHostContent`, `PlayerSubmit`'s `TimerBar`) and had already split 2-2 on the interval — one of the 1000 ms copies carrying a comment asserting the very reasoning that causes the stutter.
...
- `src/lib/ai-judge.ts` — Dual-mode judge; the live path is a **debiased, multi-call** judge.
  ...
  - A call that times out, is truncated (`max_tokens`), refuses, returns non-JSON, or fails validation is **dropped and the others carry the round** (`[ai-judge] call N dropped: …`, `degraded: k/K calls valid`). Only when none survive does it fall back to `fallbackJudge()`, the same fallback used when the key is absent — and it is **not** a coin toss: it tiers the round by the two signals available offline (is the played scheme on this challenge's list, and did the player write anything at all), then shuffles within a tier with a real Fisher–Yates (the old comparator shuffle favoured submission order). No tier depends on submission order or player id. Note what the mapping is *not*: all 30 lists are stored ascending by scheme id, so its order carries no relevance ranking — treating it as a set, which both the prompt's yes/no flag and this tiering do, is the correct reading. Nothing on the projector distinguishes a fallback verdict from a real one — check the Vercel logs for `[ai-judge] Live verdict via …` (player count, calls valid, mode, latency, winner votes) vs `[ai-judge] Claude call failed/timed out` (fallback). Every line carries a `[CODE:round]` tag; `seed … labels ANS-42=<playerId>,…`, per-call `call i/K ok … top=<label> winnerPos=<position>/N` (a `winnerPos` that clusters at 1 over many rounds is residual primacy) and `final: <label>:<mean>/<votes>/<rank>` let you reconstruct any disputed verdict.
...
- `src/lib/scheme-details.ts` — Which scheme cards have a full infographic guide, and where it lives in a given language. `hasSchemeDetail(id)` gates the Explore entry point; `schemeDetailImage(id, lang)` returns `/scheme-details/<id>.webp` or `/scheme-details-hi/<id>.webp`, or **`null`** when there is none. The id list is explicit rather than "assume every card has one": `s018` (Digital India) has no guide in **either** language — both source PDFs put the much narrower *Digital India Internship Scheme* on that page — and a missing image would otherwise render as a broken box inside the sheet, so callers must treat `null` as "no guide" and hide the entry point. Because the coverage is identical, **one** list serves both decks; do not split it into a list per language unless they actually diverge (two hand-maintained copies of the same 74 ids is bug #25's arrangement). There is deliberately **no cross-language fallback** — a guide is a full page of set text, so serving the Hindi page to someone who asked for English does not degrade gracefully; falling back to `CardModal` does. `scripts/build-scheme-details.mjs` fails if the list drifts from either map.

- `src/lib/explore-lang.ts` — `exploreLang(fallback)` / `setExploreLang(lang)`: which language the Explore screens are in, remembered per device. Same shape and the same reason as `sound-pref.ts` — `localStorage` **throws** on a device that blocks site data, so every access is try/caught and the preference fails back to English rather than taking the screen to the error boundary. Read it in an **effect**, never at render time: the server cannot know the stored value, so using it for the first paint is a hydration mismatch (bugs #9 and #10).

- `src/lib/explore-copy.ts` — Every word the Explore screens put on screen, in both languages, as a `Record<GuideLang, ExploreCopy>` — so a string added to one language and forgotten in the other is a **compile error**, not a stray English sentence in the middle of a Hindi page. Scheme names are not in here (they live with the cards, as `name` + `hi`), and neither is the card art: `public/cards/*.webp` stays English-faced in both modes because it is the physical card a player is holding at the table.

### API
- `src/app/api/game/route.ts` — Single POST + GET endpoint.
  - `create-room` — creates room, returns `{ room }`
  - `join` — blocks during `submission` and `game-over` phases; idempotent on rejoin (returns existing room). New arrivals are capped at `MAX_PLAYERS` (30); the existing-seat and stale-seat-reclaim branches sit **above** the check, so reconnection still works at a full table
...
### Explore Components
- `src/components/explore/ExplorePage.tsx` — "Know Your Deck": the deck grid, search, and the **EN ⇄ हिंदी switch** in the sticky header. The switch is labelled in the language it takes you *to* and never translated — someone who cannot read the current language has to be able to recognise their own. Hindi mode makes Hindi dominant everywhere the page speaks: the headline and its gloss swap rank (and swap faces — Bebas carries no Devanagari, so a Hindi headline set in it silently falls through to a generic sans), the tile overlay leads with the Devanagari name, and the reader is handed the Hindi deck. The language lives in `useState('en')` and is swapped in an effect, for the hydration reason in `explore-lang.ts`. Search always matches **both** scripts in both modes: a Hindi reader with a Latin keyboard types "kisan". Tapping a card measures its tile and **flies it** into `SchemeGuideReader`, which mounts a fixed clone at that rect and animates it onto the rail (X and scale on one box, Y and the settle on another, two curves, so it reads as an arc rather than a diagonal slide). The deck stays mounted for 380 ms and fades out *above* the reader — cutting it on the frame the clone appears is what made the takeoff feel abrupt. The reader's arrow keys traverse the **filtered** list minus the cards with no guide, so it only ever pages to something that exists. `CardModal` survives as the fallback for a card with no guide (`hasSchemeDetail` false — currently only `s018`); for every other card it restated exactly what the card face already prints.
- `src/components/explore/SchemeGuideReader.tsx` — Full-screen scheme guide: the card beside the page, on a designed ground (tricolour ribbon, dot grid, vignette, drifting motes), built from the Claude Design handoff. Motion lives in `globals.css` as the `vk-*` keyframes. Things to preserve, most of them learned the hard way:
  ...
  - **The heading is ONE component.** `SchemeHeading` serves both the desktop rail and, below 900px where the rail is hidden, the phone's well. They were two hand-written copies, and that is exactly how the Hindi port shipped a phone reader whose heading was still English next to a Hindi guide page. It also owns the two typographic facts that make a bilingual headline work: Bebas has no Devanagari (so Hindi headlines take the Devanagari face, a size smaller, which is where the two optically match), and Devanagari needs a taller line box (at `lineHeight: .93` the matras collide with the line above).
  ...
  - **Traversal is asymmetric, and the two objects deal in OPPOSITE directions.** Forward, the collectible leaves up-and-**left** (it rests on the left rail) while the guide page leaves up-and-**right** (it sits in the right-hand well) — each exits toward the edge it is already nearest, so the pair splits apart — and the next page settles from underneath; backward, nothing is dealt: each returns along the path it took (card from the left, page from the right) and lands on top, covering a page that does not move. The card and the page therefore need **separate** keyframe sets (`vk-deal-out`/`vk-deal-back` vs `vk-deal-out-pg`/`vk-deal-back-pg`); reusing the card's for both sends them off as one lump and the "dealt off the stack" read is gone. Nothing fades, and whichever object is moving is in front. Symmetrise it and it reads as a crossfade. The `-2` keyframe aliases exist because an unchanged `animation-name` never restarts, so two moves the same way would otherwise do nothing the second time. The collectible's float must stay on a different element from the deal, or the later `transform` wins and the deal is discarded.
...
- `context/scheme_details_map_hi.json` — the same thing for the Hindi edition (`Schemes_Hindi_4`, 87 pages): cover, title, 7 contents pages, **schemes on pages 10–84 in exact card order** (page 10 = `s001`, unlike the English deck's own order), then back matter. Page 27 is the *Digital India Internship Scheme* again, so `s018` is unmapped here too — the same 74 cards, reached by a different page order.
- `public/scheme-details-hi/<schemeId>.webp` — the Hindi guides, 74 pages, same 921×1650 / q88 (~223 KB each, ~16 MB). Served `unoptimized` like the English set.
...
41. **The fallback judge threw away a signal it already had** — the office's problem→scheme mapping is compiled into memory for the live judge's on-brief flag, but `fallbackJudge` — which decides *every* round when `ANTHROPIC_API_KEY` is absent, and any round where all three live calls fail — ignored it and shuffled. Fixed: the round is tiered by whether the played scheme is on this challenge's list and whether the player wrote anything at all (timer expiry auto-submits a random card with an empty explanation, which should never beat someone who made a case), then shuffled within a tier. The anti-bias property is intact — no tier depends on submission order or player id.

42. **Every unsubmitted phone auto-submitted at the same instant** — `PlayerSubmit` used a fixed `timerEndsAt − 1500 ms` lead, and each submit serialises through the room's write lock (which gives up after ~1 s). At a full table the tail of that burst could be rejected, `PlayerView` deliberately stays silent on an auto-submit failure, and the server's own expiry safety net then played a **random** card with no explanation for a player who had chosen one and typed a case. Fixed: the lead is jittered over ~800 ms, and a 409 now retries once.

43. **A device with a fast clock burned its whole turn on the first frame** — `timerEndsAt` is the server's clock; a client running ahead by more than the round length computed a negative delay the moment the phase opened and fired immediately, with nothing selected. The player saw "Time's up" for the rest of the round having never seen the challenge. Fixed: a delay outside the round's own length is not trusted, and the countdown falls back to the full duration from first local observation — which needs no clock agreement at all.

44. **`timer-expire` took the room write lock before checking whether there was anything to do** — in normal play every player *and* the projector fire it within milliseconds, so N+1 callers queued for the lock, contending with the submissions still trying to land, only to find the phase had moved on. It is also unauthenticated, so a loop against a known 4-char code could starve real submits into 409s. Fixed: an unlocked pre-check bails out the callers with nothing to do (with a 1 s margin so the marginal case keeps the serialize-and-recheck path), plus a 40/60 s per-room cap.

45. **A room could be filled until it stopped working** — no cap on `join`. Each seat adds ~2.5 KB of hand JSON to the room's single Redis value; past ~400 it exceeds Upstash's 1 MB request limit and then *every* mutating action throws, including `end-game`, so the host cannot even close the room. Capped at 30, above the ~20 a venue seats, with the existing-seat and reclaim branches exempt so reconnection still works at a full table.

46. **The reader's arrows looked disabled without being disabled** — `Circle` used its `disabled` prop only for `opacity` and `cursor`, so an arrow at 0.35 opacity still fired `go()` and assistive tech announced it as enabled. The edge flare could also stick: bouncing off the last scheme queued a reset for +420 ms, and traversing the other way inside that window cleared all pending timers without clearing the flare. Both fixed.

47. **The guide page dealt off in the same direction as the card** — the design's traverse splits the pair: the collectible exits up-and-left toward its rail, the guide page up-and-right toward its well, and backward each returns along its own path. The port reused the card's left-going keyframes for the page, so both objects slid off to the left together and the deal read as one lump sliding away rather than a card being dealt off a stack. Fixed: `vk-deal-out-pg`/`vk-deal-back-pg` (`translate(118%, -7%) rotate(9deg)`) added and the page repointed at them. Measured on a clean build during a forward traverse: card 279 → 115 (left), page 757 → 856 (right); backward, card arrives from 244 and the page from 854. Note the build check — Next 16 emits CSS to `.next/static/chunks/`, not `.next/static/css/`, so grep the right path when confirming a keyframe actually shipped.

48. **A prototype key was a valid player id** — `room.players[playerId]` was a truthiness read and `tokenOk` treated a falsy `room.tokens?.[playerId]` as "no token issued, allow". `createRoom` never initialised `tokens`, so between room creation and the first join, `{"action":"join","playerId":"__proto__"}` took the seat branch and ran `room.players['__proto__'].lastSeen = Date.now()` — a remote, unauthenticated write to `Object.prototype` for the life of a serverless instance shared by every room. In the same window the public `playerId` was the only credential for any seat, returning that player's private hand and rotating their token. Fixed: `Object.hasOwn` at every membership and token lookup, `tokens: {}` in `createRoom`, and a format + reserved-name guard on `playerId`/`hostId` at the top of POST. Note the first guard was **not enough on its own** — `__proto__`, `constructor` and `toString` are all letters and underscores and passed the character class; they are rejected by name.

49. **The profanity filter missed the plural of every word it blocked** — `\bword\b` against a list of bare roots. Verified passing uncensored, onto a projector at a government-adjacent event: `fucking`, `fucks`, `assholes`, `shitty`, `bitches`, `chutiye`, `behenchod`, `madarchodon`. The model's own output was never filtered at all, and the system prompt explicitly asks the judge to quote a phrase from the player's explanation. Fixed: explicit per-root suffix sets, Cyrillic homoglyph folding, full-word censoring (`s**t` is perfectly legible from the back of a hall), and `filterText` applied to `judgeComment`/`reasoning`.

50. **Every length cap was bypassable ~6x because NFKC ran after truncation** — `filterText` normalises, which expands; callers sliced first. Verified: 30 squared-katakana characters stored a 180-character name, and 200 stored a 1200-character explanation whose word count was 1, sailing past the 25-word check. The same ordering silently undid `sanitizeName`'s `<>` strip, because NFKC folds fullwidth `＜＞` to ASCII *after* the strip had run. Fixed by normalising first and capping after, in `sanitizeName` and at both other call sites.

51. **Unauthenticated callers took the room write lock before being authenticated** — `heartbeat`, `chat` and `submit` all did `withRoomLock` first and checked identity inside. `withRoomLock` polls ~1 s with no queue, so a flood of unauthenticated heartbeats starves real writers. That is not just a DoS: `PlayerView` beats every 20 s fire-and-forget, so ~a minute of starvation drops three beats, a player sitting there with their phone open goes `lastSeen`-stale, and the name-based reclaim then hands their seat, hand and score to anyone who can read their name off the projector. The reclaim tradeoff assumes the player actually left; this made **any seat stealable on demand**. Fixed: authenticate on an unlocked read before queuing for the lock, plus per-room caps.

52. **Admin `DELETE` was a better brute-force door than the admin API** — `/api/admin` is throttled; `DELETE /api/game?code=` ran Basic Auth and nothing else, giving unlimited guesses and a clean oracle (401 wrong, 400/404 right). Fixed: charged per attempt, keyed on the IP *and* a hash of the supplied credential, so a wrong password costs the guesser wherever it comes from and a venue sharing one egress IP cannot lock the operator out.

53. **The Pusher auth endpoint was unrate-limited** — it proves only that a room exists, so one laptop could open authorised sockets until the plan's concurrent-connection cap was exhausted, dropping every real player to polling. Now capped per IP. It still does not prove membership, and cannot: the projector is a legitimate subscriber with no seat. See the route's own comment for what that does and does not expose.

54. **A missing `ANTHROPIC_API_KEY` was invisible in production** — `env.ts` only warns about optional variables in development, and `judgeRound` took the keyless path with no log at all. Combined with nothing on the projector distinguishing a fallback verdict from a real one, a key that was missing or expired would have every round of a public event decided locally with nobody any the wiser. It now logs loudly, every round.

55. **No security headers at all** — no CSP, `X-Frame-Options`, `Referrer-Policy` or HSTS anywhere. `Referrer-Policy` matters here specifically: the host's credential lives in `/projector/CODE?h=<hostId>` until the client strips it. Added in `next.config.ts`, deliberately conservative — the UI is built on inline `style={{}}` and Next injects inline bootstrap scripts, so a strict `script-src`/`style-src` without nonces would break the game. Verified all five routes render with no CSP violations.

56. **The admin `DELETE` rate limit did not limit anything** — bug #52's own fix. The bucket key was `sha256(ip | Authorization header)`, reasoning that keying on the IP alone would let a venue behind one egress IP lock the real operator out. But the `Authorization` header is **attacker-controlled**, and a brute-forcer varies the password on every attempt *by definition* — so every guess landed in its own fresh 10-attempt bucket and the limit never engaged. Measured: 16 consecutive wrong passwords, zero 429s, while the PR that shipped it claimed "a wrong password costs the guesser wherever it comes from". Fixed by counting only **failed** attempts, keyed on the IP alone: a correct credential is never counted and never blocked, whatever the bucket holds, so nobody on the shared Wi-Fi can throttle a host who actually knows the password. Re-measured: 429 at attempt 11 with a different password every time, and the operator's correct credential still returns 200 straight after the bucket is burned.

57. **Host-only actions took the room write lock before checking the credential** — `advance`, `update-settings`, `end-game` and `kick-player` all called `withRoomLock` and checked `hostId` *inside* it, the shape bug #51 fixed for `heartbeat`/`chat`/`submit` but never applied here, and none of the four was rate limited. Anyone who can read the code off the projector could make the server acquire the room lock and read Redis on every request, unboundedly. **What this is not:** measured at 15 ms of simulated Redis latency, 400 concurrent credential-less `advance` calls held the lock ~78% of the time and all 30 concurrent legitimate player actions still succeeded — `withRoomLock`'s 40×25 ms retry budget finds a gap, and the attacker's own requests serialize through the very lock they are monopolising. So it was cost and contention, not a denial of service. Fixed with `hostPreCheck`: credential proven on an unlocked read, then a per-host 120/min bucket charged *after* the match (charging before it would let a stranger burn the real host's quota). Re-measured at the same latency: 400 bogus calls, 0 lock acquisitions, 0 "Room busy" where there had been 263.

58. **The framework itself was the biggest hole** — `next` was pinned at `16.2.6`, inside the affected range of a critical advisory set that includes *Unauthenticated Remote Code Execution in the Image Optimization API when AVIF files are used* (GHSA-2xp9-vwfh-vxw4, patched in 16.3.3) — and `next.config.ts` enables `formats: ['image/avif', …]`. Exploitability on this deployment is low: there are no `remotePatterns`, so `/_next/image` only accepts same-origin paths, every asset is committed WebP, and nothing accepts an upload — but the audit also flagged POST-body cache confusion, an image-optimizer SVG DoS, and `sharp`/libvips and libheif CVEs underneath. No security pass is complete while `npm audit` reports a critical in the framework serving every request. Bumped to `16.3.5` plus `npm audit fix` for the transitive set: **0 vulnerabilities**, and re-verified with a full two-round game, the whole previous attack suite, and all nine routes rendering with no CSP violations. (`next dev` in 16.3 also rewrites its managed block in `AGENTS.md`; that edit is committed deliberately, not stray.)

59. **CI handed the release keystore's passwords to every later step** — `android-apk.yml` wrote `STOREPASS`/`KEYPASS` into `$GITHUB_ENV`, which makes them environment variables for the rest of the job, including `bubblewrap update`/`build` and every Gradle plugin and transitive download those pull in. GitHub masks the values in the *log*; it does nothing about the process environment. Now only a `THROWAWAY` flag crosses `$GITHUB_ENV` and each step that genuinely needs a password takes it from `secrets.*` scoped to that step. `ci.yml` also gained `permissions: contents: read` — it only runs the repo's own checks, and was inheriting the repository default.
...
### Security residuals (deliberate)
- **The Pusher auth endpoint cannot prove room membership** — and will not, without moving chat to its own channel. The projector is a legitimate subscriber with no seat and no credential, and `/projector/[code]` without `?h=` is supported, so anyone who can read the code off the big screen can subscribe. `stripForBroadcast` means the feed carries only what the projector is already displaying; the one stream that is *not* on the big screen is `game:chat`. See the route's own comment.
- **GET `/api/game?code=` is unauthenticated and deliberately NOT rate limited** — it is a room-existence oracle over the 456,976-code space and one Redis read per call. A limiter is the obvious fix and is the wrong trade: `checkRateLimit` costs its own Redis round trip, so it would take the *normal* path from 1 command to 3 — a 20-player venue polls ~420 times a minute — to bound an abnormal one. The room code is not a secret anyway (it is printed on a projector). Revisit if Upstash billing ever shows it.
- **`checkRateLimit` fails open** — a Redis error returns "within limit" rather than blocking. Correct for a party game (a Redis blip must not stop a round) but it means every limit above is best-effort, not a guarantee.

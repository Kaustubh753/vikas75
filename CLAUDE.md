...
- `src/lib/ai-judge.ts` — Dual-mode judge; the live path is a **debiased, multi-call** judge.
  ...
  - Cost: three calls per round instead of one; the ~1.5k-token system prompt is sent with `cache_control` so repeat rounds read it from cache (`cacheRead=` in the per-call log shows the hit rate). Roughly a few cents per round at a full table.
